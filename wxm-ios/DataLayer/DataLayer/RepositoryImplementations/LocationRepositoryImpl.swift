...
extension SearchError {
    func toDeviceLocationError() -> DeviceLocationError? {
        ...
        case .incorrectEventTemplate, .incorrectSearchResultForFeedback, .categorySearchRequestFailed(reason: _), .failedToRegisterDataProvider(reason: _, dataProvider: _), .internalSearchRequestError(message: _), .dataResolverNotFound:
